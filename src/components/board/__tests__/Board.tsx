...
describe("cells", () => {
    ...
    it("sets a cell to 'X' when it is clicked. \
        Clicking twice on the same cell doesn't make any difference. \
        sets a cell to 'O' when a new cell is clicked.", () => {
        render(<Board />);
...
describe("player", () => {
    it("starts out with player X then switches to player O", () => {
        render(<Board />);
...
test("switches between players", () => {
        ...
        // Pick another cell to click. This should result in a 'O'
        fireEvent.click(screen.getAllByTestId('cell-click')[5]);
        expect(screen.queryAllByText('O')).toHaveLength(1);
        expect(screen.queryAllByText('X')).toHaveLength(1);
})